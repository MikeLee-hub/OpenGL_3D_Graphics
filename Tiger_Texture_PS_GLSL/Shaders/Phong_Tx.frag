...
#define NUMBER_OF_LIGHTS_SUPPORTED 8
...
uniform int u_blind_num = 1;
uniform float u_fragment_alpha = 1;
uniform float u_flag_blending = 0;
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if (!u_blind_effect || i!=1) {
						...
						tmp_float2 = (u_blind_num % 60);
						if (tmp_float2 > 30)
							tmp_float2 = 60 - tmp_float2;
						tmp_float = cos(tmp_float2*acos(tmp_float));
						if (tmp_float < 0.5) 
							tmp_float = zero_f;
					}
				}
...
#define FOG_COLOR vec4(0.5f, 0.5f, 0.7f, 1.0f)
#define FOG_NEAR_DISTANCE 1500.0f
#define FOG_FAR_DISTANCE 2500.0f
...
void main(void) {
	...
	if (screen_effect) {
		...
		if(y_mod>x_mod-0.5 && y_mod<x_mod+0.5 && y_mod<1.5-x_mod && y_mod>0.5-x_mod)
			discard;
...
// For a normal rendering ...
	...
	if (u_flag_fog) {
		int tmp_fog = u_blind_num % 90;
		if (tmp_fog > 45)
			tmp_fog = 90 - tmp_fog;
		tmp_fog *= 100;
 	  	fog_factor = (FOG_FAR_DISTANCE - length(v_position_EC.xyz))/(FOG_FAR_DISTANCE - FOG_NEAR_DISTANCE + tmp_fog);  		
